<footer style="background:#0A2472; color:white;" class="text-white py-5">
  <div class="container py-5">

    <div class="row">

      <!-- Kolom 1: Logo & Identitas -->
      <div class="col-md-5 mb-4">
        <div class="d-flex align-items-center mb-3">
          <img src="/lab-ai/assets/img/banners/logo-jti.png" 
               alt="Logo JTI" 
               style="height: 80px;">
        </div>

        <h5 class="fw-bold m-0">JURUSAN TEKNOLOGI INFORMASI</h5>
        <h5 class="fw-bold">POLITEKNIK NEGERI MALANG</h5>
      </div>

      <!-- Kolom 2: Kontak -->
      <div class="col-md-4 mb-4">
        <h5 class="fw-bold">KONTAK :</h5>
        <p class="m-0 mt-3">+62 21 1234 5678</p>
        <p class="m-0">
          <a href="mailto:contact@jti.polinema.ac.id" class="text-white text-decoration-none">
            contact@jti.polinema.ac.id
          </a>
        </p>

        <p class="mt-3">
          Jalan Soekarno-Hatta No. 9,<br>
          Kelurahan Jatimulyo, Kecamatan<br>
          Lowokwaru, Kota Malang, Jawa Timur<br>
          65141
        </p>
      </div>

      <!-- Kolom 3: Halaman -->
      <div class="col-md-3 mb-4">
        <h5 class="fw-bold">HALAMAN :</h5>

        <ul class="list-unstyled mt-3">
          <li><a href="/lab-ai/index.php" class="text-white text-decoration-none">Home</a></li>
          <li><a href="/lab-ai/public/proyek.php" class="text-white text-decoration-none">Proyek</a></li>
          <li><a href="/lab-ai/public/publikasi.php" class="text-white text-decoration-none">Publikasi</a></li>
        </ul>
      </div>

    </div>

    <hr class="border-light mt-4">

    <div class="text-center py-2">
      <small>&copy; <?= date('Y') ?> Laboratorium for Applied Informatics</small>
    </div>

  </div>
</footer>
